...
func contentTypeOptions(next http.Handler) http.Handler {
	return http.HandlerFunc(func(responseWriter http.ResponseWriter, request *http.Request) {
		// Security Misconfiguration – The system is deployed or configured in an unsafe way.
		// One simple way to fix this is all {"X-Content-Type-Options": "nosniff"} header
